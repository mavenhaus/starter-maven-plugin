...
assert 'optional-elements-modeMinimum' == resolvedProject.artifactId.text()
assert '0.0.1-SNAPSHOT' == resolvedProject.version.text()
// banned elements for artifact
assert 0 == resolvedProject.build.size()
assert 1 == resolvedProject.ciManagement.size()
assert 1 == resolvedProject.contributors.size()
assert 1 == resolvedProject.dependencyManagement.size()
assert 1 == resolvedProject.description.size()
assert 1 == resolvedProject.developers.size()
assert 1 == resolvedProject.distributionManagement.size()
assert 1 == resolvedProject.issueManagement.size()
assert 1 == resolvedProject.mailingLists.size()
assert 0 == resolvedProject.name.size()
assert 1 == resolvedProject.organization.size()
assert 0 == resolvedProject.parent.size()
assert 1 == resolvedProject.pluginRepositories.size()
assert 1 == resolvedProject.repositories.size()
assert 1 == resolvedProject.prerequisites.size()
assert 1 == resolvedProject.properties.size()
